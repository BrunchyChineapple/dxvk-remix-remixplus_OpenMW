...
// rtx_weather.h -- weather preset declarations.
//
// Defines 852 RTX_OPTIONs (12 presets x 71 fields) under the rtx.weather.preset.<presetName>
// namespace. 63 of those fields are upstream's; the remaining 8 are this fork's time-of-day and
// underwater fog split, which upstream's model has no equivalent for:
//
//   fogDensityReferenceTransmittance{Day,Night,UnderwaterDay,UnderwaterNight}
//   transmittanceMeasurementDistanceMeters{Day,Night,UnderwaterDay,UnderwaterNight}
//
// Both counts in the pre-merge headers were stale after the 2026-08-18 atmosphere merge -- ours
// said 67 fields and upstream's said 63, and the merged table is 71. Stated here explicitly
// because the number is load-bearing: WeatherSnapshot, the blender's lerp loop and the per-preset
// option declarations are all generated from this one table.
//
// Usage: invoke DECLARE_ALL_WEATHER_PRESETS() inside the RtxOptions struct body. The macro expands
// all 12 preset declarations inline.
//
// WEATHER_PRESET_FIELD_LIST(X) is the single source of truth. DECLARE_WEATHER_PRESET delegates to
// it via a per-preset binder, so adding a field requires one edit here and the 12 preset
// declarations, the snapshot members and the ownership mask all regenerate automatically.
...
#undef WEATHER_PRESET_FIELD_AS_OWNERSHIP_
  ...
  // -------------------------------------------------------------------------
  // WeatherSnapshot -- a plain-value copy of all 71 effective weather params,
  // plus ownership metadata for UI/debug consumers.
  //
  // Members are auto-generated from the single-source-of-truth X-macro, so any
  // field addition propagates here for free. Renderers consume only the values;
  // ImGui uses the ownership mask to decide whether to show a read-only weather
  // value or the editable base RtxOption, without reintroducing transient
  // Derived-layer writes.
  // -------------------------------------------------------------------------

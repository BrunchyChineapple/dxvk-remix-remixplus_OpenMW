...
#undef WRF
  ...
  //
  // The four Day/Night/UnderwaterDay/UnderwaterNight measurement distances treat <= 0 as "inherit"
  // rather than as a literal distance, so a preset only has to state the ones that differ from its
  // base. This resolves them on the way out of a preset read, so no zero ever reaches the blender:
  // these fields are WK_Extinction and lerp in 1/distance space, where a 0 clamps to 1e-4 m and
  // would read as fully opaque for the duration of a weather transition.
  //
  // Resolving after blending would be worse: a preset pair of (inherit, 500) has no meaningful
  // midpoint until both sides are concrete.
  //
  // Underwater falls back through the above-water day/night value rather than straight to the base,
  // so a preset that splits its air fog by time of day gets that split in its water for free and
  // only needs the underwater fields when the water should differ from the air.
  //
  // Deliberately not applied in copyPresetToPreset below: that copies preset to preset, and baking
  // the resolved number into the destination would silently convert "inherit" into a literal.
